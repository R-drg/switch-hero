...
class Controller {
  ...
  private:
    ...
    struct Snapshot {
        ...
        // Whammy sources: each pad's further-pushed stick, and the guitar's bar,
        // which the MissionControl patch sends as right stick X. Both 0 to 1.
        float standardStick[maxPlayers] = {}, guitarWhammy = 0;
    };
    ...
    void poll() {
        ...
        while (!stopping) {
            ...
            for (size_t p = 0; p < maxPlayers; ++p)
                s.standardStick[p] = std::max(deflection(padGetStickPos(&nx[p], 0)), deflection(padGetStickPos(&nx[p], 1)));
    ...
    void vibrate(float low, float high) {
        const uint32_t style = [&] {
            std::lock_guard<std::mutex> lock(mutex);
            return snap.standardStyle[0] | snap.guitarStyle; // rumble is player one's
        }();
  ...
  public:
...
#else
        if (pad[0])
            SDL_GameControllerRumble(pad[0], Uint16(strength * .6f * 0xffff), Uint16(strength * 0xffff),
                                     Uint32(seconds * 1000));
#endif
    }
    ...
    Probe probe() {
        ...
        p.name = pad[0] ? SDL_GameControllerName(pad[0]) : tr("no controller");
    ...
    uint64_t read(bool wiiGuitar, bool playing, double *pressedAt = nullptr, size_t player = 0,
        ...
        if (whammy)
            *whammy = player == 0 && guitarInput ? s.guitarWhammy : s.standardStick[player];
            ...
            *whammy = 0;
            if (auto *p = pad[player])
                for (auto [x, y] : {std::pair{SDL_CONTROLLER_AXIS_LEFTX, SDL_CONTROLLER_AXIS_LEFTY},
                    ...
                    *whammy = std::max(*whammy, std::min(1.0f, std::hypot(float(SDL_GameControllerGetAxis(p, x)),
                                                                         float(SDL_GameControllerGetAxis(p, y))) /
                                                                  32767.0f));
        }
        if (auto *p = pad[player]) {
            for (int b = 0; b < SDL_CONTROLLER_BUTTON_MAX; ++b)
...
// (Named apart from look::Board, which picks the highway's surface.)
struct BoardShape {
...
void highway(const Song &song, const Session &session, const Settings &settings, double time, uint8_t held,
    ...
    // board tapered.
    const float top = shape.top, bottom = shape.bottom, edge = shape.edge, nearWidth = shape.nearWidth,
                farWidth = shape.farWidth;
...
std::vector<OptionRow> optionRows(int page, const Settings &s, bool confirmReset) {
    ...
    if (page == 0) {
        ...
        rows.push_back({Opt::FilmGrain, 0, tr("Film grain"), tr(s.filmGrain ? "ON" : "OFF"),
                        tr("The moving speckle over the picture. Turn it off first if frames drop."), true});
...
int main(int argc, char **argv) {
    try {
        ...
        enum class Screen { Language, Main, Library, Select, Playing, Paused, Countdown, Results, Settings, Calibrate, Download, Sections, Lobby };
        ...
        auto startMulti = [&]() {
            ...
            for (size_t p = 0; p < seats.size(); ++p) {
                ...
                if (index < 0) {
                    message = tr("Player {} has no chart for that difficulty", p + 1);
            ...
            if (tracks.size() < 2) {
                message = tr("Multiplayer needs at least two players");
                audio.playSfx(Sfx::Back);
                return;
            }
            try {
        ...
        while (running) {
            ...
            uint64_t buttons = controller.read(settings.wiiGuitar, screen == Screen::Playing || screen == Screen::Calibrate,
                                               &pressedAtSeconds, 0, &whammyInput),
            ...
            if (screen == Screen::Language) {
                ...
                // Quickplay, practice, multiplayer, download, options, quit.
                constexpr int quitRow = 5;
                if (up || down) {
                ...
                if (back && mainRow != quitRow) {
                    ...
                    if (mainRow <= 1) {
                        ...
                        if (!platform::docked()) {
                            audio.playSfx(Sfx::Back);
                            message = tr("Multiplayer needs the console in its dock");
                        } else {
                            audio.playSfx(Sfx::Select, .7f);
                            message.clear();
                            practiceMode = false;
                ...
                if (!platform::docked()) {
                    ...
                    audio.pause(true);
                    audio.playSfx(Sfx::Back);
                    message = tr("Dock the console to carry on");
                    screen = Screen::Paused;
                } else if (pause) {
                    frozen = time;
                    audio.pause(true);
                    ...
                    for (size_t i = 0; i < multi->size(); ++i) {
                        ...
                        if (p.combo / 50 > seatMilestone[i]) {
                            ...
                            seatCallout[i] = tr("{} NOTE STREAK!", seatMilestone[i] * 50);
                            seatCalloutAt[i] = now();
            ...
            if (screen == Screen::Language) {
                ...
                menu(330, 316, 52,
                     {tr("QUICKPLAY"), tr("PRACTICE"), tr("MULTIPLAYER"), tr("DOWNLOAD SONGS"), tr("OPTIONS"), tr("QUIT")},
                     mainRow, ui, 400, 28);
                static const char *const blurbs[] = {"pick a song from your library", "loop a section until you nail it",
                                                     "two to four players, split screen", "grab charts from chorus encore",
                                                     "controls, calibration and gameplay", "back to the homebrew menu"};
                auto blurb = body(18, ink::dim);
                blurb.align = Align::Center;
                // Say why the multiplayer entry will refuse before it is picked, not after.
                text(330, 620, tr(mainRow == 2 && !platform::docked() ? "dock the console to play multiplayer" : blurbs[mainRow]),
                     blurb);
                if (!message.empty())
                ...
                wall(ui, ink::crt);
                text(58, 24, tr("MULTIPLAYER"), stencil(68, ink::chrome, {116, 122, 138, 255}));
                {
                ...
                for (size_t p = 0; p < seats.size(); ++p) {
                    ...
                    text(x + 24, y + 18, tr("PLAYER {}", p + 1), stencil(30, ink::chrome, ink::steel));
                    if (!controller.connected(p)) {
                        text(x + 24, y + 80, tr("no controller"), marker(26, ink::faint, -2));
                        continue;
                    }
                    if (!seat.joined) {
                ...
                for (size_t i = 0; i < n; ++i) {
                    ...
                    // close the meter is to the floor.
                    {
                        ...
                        text(46, 274, tr("{} streak", p.combo),
                             marker(24, p.combo ? ink::acid : ink::dim, -3));
                        const int judged = p.hits + p.misses;
                        text(46, 306, tr("{}% hit", judged ? p.hits * 100 / judged : 100),
                             body(20, ink::faint));
                    ...
                    if (settings.hitRatings && p.lastTierAt > -1e8) {
                        ...
                        if (pop > 0) {
                            ...
                            if (p.lastTier < 3) {
                                ...
                                l.align = Align::Center;
                                text(640, 516, tr(p.lastError < 0 ? "EARLY" : "LATE"), l);
                            }
                        }
                    }
                ...
                if (screen == Screen::Paused || screen == Screen::Results) {
                    ...
                    text(640, 40, tr(screen == Screen::Paused ? "PAUSED" : "FINAL STANDINGS"), heading);
                    if (screen == Screen::Paused)
                        menu(640, 300, 62, {tr("RESUME"), tr("RESTART"), tr("QUIT TO SONG LIST")}, pauseRow, ui, 560, 34);
                    if (screen == Screen::Results) {
                        ...
                        for (size_t r = 0; r < board.size(); ++r) {
                            ...
                            text(240, y + 16, tr("PLAYER {}", seat + 1), stencil(34, ink::white, ink::dim));
                            ...
                            text(1100, y + 14, tr("{}% hit", int(row.accuracy * 100)), detail);
                            text(1100, y + 44, tr("{} streak", row.maxCombo), detail);
